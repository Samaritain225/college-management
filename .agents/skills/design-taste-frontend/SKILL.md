...
This skill's own scope note ("Not dashboards, not data tables, not
multi-step product UI") applies literally here — Wagnon Budget is exactly
that: a data-table-heavy finance dashboard, not a landing page or portfolio.
Use only the **redesign - preserve** mode (Section 1.B / 1.A "redesign -
preserve" row) and the general anti-templating discipline (Section 0.D, taste
over defaults). Skip the dial presets for landing/portfolio/editorial pages
entirely, and don't reach for a new design-system package (Section 2) — this
project already has one (Tailwind v4 tokens in `src/index.css` + shadcn-style
primitives in `src/components/ui/`); extend it, don't replace it.
