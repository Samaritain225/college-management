-- Part 3 of docs/access-lifecycle-plan-2026-07-29.md: let the client render
-- "(compte supprimé)" next to a name whose account was soft-deleted, instead
-- of silently showing a name with no indication the account behind it is
-- gone. Same signature as the current expenses_page (create or replace, no
-- new parameters) — this is additive JSON only, so no overload to manage.
